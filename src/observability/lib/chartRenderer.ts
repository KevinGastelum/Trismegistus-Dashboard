import type { ChartDataPoint, ChartConfig, ChartDimensions } from "./types";
...
export class ChartRenderer {
  ...
  setDark(v: boolean) { this.isDark = v; }
  setConfig(config: ChartConfig) { this.config = config; }
  ...
  clear() { this.ctx.clearRect(0, 0, this.dimensions.width, this.dimensions.height); }

  drawBackground() {
    const chartArea = this.getChartArea();
    const gradient = this.ctx.createLinearGradient(chartArea.x, chartArea.y, chartArea.x, chartArea.y + chartArea.height);
  ...
  drawTimeLabels(timeRange: string) {
    ...
    labels.forEach((label, index) => {
      this.ctx.fillText(label, chartArea.x + index * spacing, chartArea.y + chartArea.height + 5);
    });
  }
    ...
    dataPoints.forEach((point, index) => {
      ...
      if (formatLabel && point.eventTypes && Object.keys(point.eventTypes).length > 0 && barHeight > 10) {
        ...
        if (label) {
          ...
          this.ctx.fillStyle = this.isDark ? "rgba(75, 85, 99, 0.95)" : "rgba(0, 0, 0, 0.85)";
          this.ctx.beginPath();
          const ctx2d = this.ctx as CanvasRenderingContext2D & { roundRect?: (x: number, y: number, w: number, h: number, r: number) => void };
          if (typeof ctx2d.roundRect === "function") {
            ctx2d.roundRect(bgX, bgY, bgWidth, bgHeight, 7);
          } else {
            const radius = 7;
          ...
          this.ctx.fillText(label, bgX + padding, labelY);
          this.ctx.restore();
        }
      }
    });
